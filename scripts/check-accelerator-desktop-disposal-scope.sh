...
added="$(git diff --unified=0 "$base...HEAD" -- pkg/acceleratorprovision ':(exclude)pkg/acceleratorprovision/*_test.go' pkg/helm/accelerator.go pkg/helm/accelerator_cleanup.go pkg/helm/accelerator_sweep.go accelerator_dispose_desktop.go accelerator_dispose_desktop_stub.go | sed -n 's/^+//p' | grep -v '^+++' || true)"
...
if git diff --name-only "$base...HEAD" | grep -Eq '(^app\.go$|^app_|^frontend/|^pkg/server/|^deploy/charts/)'; then
