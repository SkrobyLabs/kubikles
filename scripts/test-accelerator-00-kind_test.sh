...
run() { local scenario="$1"; shift; rm -rf "$tmp/state"; mkdir "$tmp/state"; PATH="$bin" HARNESS_STATE="$tmp/state" HARNESS_FAIL="$scenario" /usr/bin/bash "$@"; }
expect() {
