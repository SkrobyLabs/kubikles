...
git merge-base --is-ancestor "$lifecycle_head" HEAD || fail "lifecycle-head-not-ancestor"
git diff --check "$base...$lifecycle_head" || fail "diff-check"
...
done < <(git diff --name-only "$base...$lifecycle_head")

added="$(git diff --unified=0 "$base...$lifecycle_head" -- '*.go' ':(exclude)*_test.go' | sed -n 's/^+//p' | grep -v '^+++' || true)"
