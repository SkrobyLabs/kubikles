...
test-accelerator-desktop-connector-kind:
	@./scripts/test-accelerator-desktop-connector-kind.sh

test-accelerator-desktop-resume-kind:
	@./scripts/test-accelerator-desktop-resume-kind.sh

test-accelerator-desktop-disposal-kind:
	@./scripts/test-accelerator-desktop-disposal-kind.sh
...
help:
	...
	@echo "  test-accelerator-00-kind  Run initial Accelerator tests in Kind"
	@echo "  test-accelerator-chart  Render and test the Accelerator Helm chart"
	@echo "  test-accelerator-chart-kind  Test the Accelerator Helm chart in Kind"
	@echo "  test-accelerator-desktop-provision-kind  Test desktop provisioning and rollback in Kind"
	@echo "  test-accelerator-desktop-connector-kind  Test an authenticated desktop connection in Kind"
	@echo "  test-accelerator-desktop-resume-kind  Test desktop session reconnection in Kind"
	@echo "  test-accelerator-desktop-disposal-kind  Test Accelerator cleanup in Kind"
	@echo "  test-accelerator-desktop-lifecycle-kind  Test the desktop Accelerator lifecycle in Kind"
	@echo "  test-accelerator-integrated-routing-kind  Test integrated Secret routing in Kind"
	@echo "  test-accelerator-release-contract  Test Accelerator registry and release packaging"
...
build-accelerator-image: accelerator-docker-preflight build-accelerator
	@port="$${KUBIKLES_ACCELERATOR_E2E_REGISTRY##*:}"; \
	  ...
	  digest="$$(jq -r .registryImageDigest '$(ACCELERATOR_ACCEPTANCE_ARTIFACT_ROOT)/acceptance-artifact-metadata.json')"; \
