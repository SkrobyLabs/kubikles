...
flowchart LR
    ...
    kbk -->|optional Secret reads| accel[Accelerator workload]
    accel -->|fixed read-only scope| api
...
Accelerator is an explicitly enabled, connection-owned companion for Secret
reads. The desktop may route Secret list, detail, and watch requests through a
disposable workload inside the selected cluster, closer to the Kubernetes API.
Every other operation—and every failed identity, access, or connection check—uses
the ordinary Direct path, which remains authoritative.

This shape keeps acceleration optional and narrowly bounded instead of turning it
into a general proxy or an availability dependency. See [Kubikles
Accelerator](features/accelerator.md) for its purpose, scope, and security
boundaries; this document deliberately does not duplicate them.
