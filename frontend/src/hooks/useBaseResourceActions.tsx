...
export function useBaseResourceActions<T extends K8sResource = K8sResource>(
  ...
  const handleShowDetails = (resource: T): void => {
    ...
    const tabId = `details-${resourceType}-${currentContext}-${isNamespaced && namespace ? `${namespace}/${name}` : name}`;
    const props = {
  ...
  const handleEditYaml = (resource: T): void => {
    ...
    const tabId = `yaml-${resourceType}-${currentContext}-${isNamespaced && namespace ? `${namespace}/${name}` : name}`;
    openTab({
      id: tabId,
      title: name || 'Unknown',
      icon: getResourceIcon(resourceType),
  ...
  const handleShowDependencies = hasDependencies
    ? (resource: T): void => {
        ...
        const tabId = `deps-${resourceType}-${currentContext}-${isNamespaced && namespace ? `${namespace}/${name}` : name}`;
        openTab({
          id: tabId,
          title: name || 'Unknown',
          icon: getResourceIcon(resourceType),
