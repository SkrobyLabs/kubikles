...
import { alignedValue, contributorSummaries, freshness, latestPoint, minimumPoint, pressureEpisodes, segmentSeries, timelineDomainMax, timelineTimeTicks, timelineTooltip, timelineValueTicks, visibleTimelineLines, type MemoryPoint, type TimelineLine } from './nodeMemoryDiagnostics';
...
function Timeline({ lines, hiddenLabels, onToggleLine, rangeStartMs, rangeEndMs, stepMs, markers = [], onZoomSelect }: { lines: Line[]; hiddenLabels: ReadonlySet<string>; onToggleLine: (label: string) => void; rangeStartMs: number; rangeEndMs: number; stepMs: number; markers?: EventMarker[]; onZoomSelect?: (startMs: number, endMs: number) => void }) {
    const plotLeft = 72;
    const plotRight = 780;
    const plotTop = 16;
    ...
    const visible = visibleTimelineLines(lines, hiddenLabels) as Line[];
    const available = visible.filter(line => line.data?.length);
    const legendLines = lines.filter(line => line.data?.length);
    ...
    const max = timelineDomainMax(available);
    ...
    return <div className="relative border border-border bg-background">
        ...
        <div className="flex flex-wrap gap-1 px-3 pb-2 text-xs">{legendLines.map(line => {
            const visible = !hiddenLabels.has(line.label);
            return <button key={line.label} type="button" aria-pressed={visible} onClick={() => onToggleLine(line.label)} className={`flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors hover:bg-white/10 ${visible ? '' : 'opacity-40'}`} title={`${visible ? 'Hide' : 'Show'} ${line.label}`}>
                <span className={`w-3 h-0.5 ${visible ? line.color.replace('stroke-', 'bg-') : 'bg-gray-500'}`} />
                <span className={visible ? line.color.replace('stroke-', 'text-') : 'text-gray-500'}>{line.label}</span>
            </button>;
        })}{!legendLines.length && <span className="text-gray-500">This diagnostic source is unavailable for the selected range.</span>}{legendLines.length > 0 && !all.length && <span className="px-1.5 py-0.5 text-gray-500">All diagnostic series are hidden.</span>}</div>
    </div>;
}
...
export default function NodeMemoryDiagnostics({ memory, contributors, rangeStartMs, rangeEndMs, stepMs, markers, onZoomSelect }: Props) {
    ...
    const [hiddenLabels, setHiddenLabels] = useState<Record<'pressure' | 'allocation' | 'breakdown', Set<string>>>(() => ({ pressure: new Set(), allocation: new Set(), breakdown: new Set() }));
    ...
    const toggleLine = (label: string) => setHiddenLabels(current => {
        const next = new Set(current[view]);
        if (next.has(label)) next.delete(label);
        else next.add(label);
        return { ...current, [view]: next };
    });
    const timeline = <Timeline lines={lines} hiddenLabels={hiddenLabels[view]} onToggleLine={toggleLine} rangeStartMs={rangeStartMs} rangeEndMs={rangeEndMs} stepMs={stepMs} markers={markers} onZoomSelect={onZoomSelect} />;
