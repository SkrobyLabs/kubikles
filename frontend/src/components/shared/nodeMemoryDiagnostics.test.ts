...
import { alignedValue, contributorSummaries, freshness, integerTimeRange, minimumPoint, nonNegative, pressureEpisodes, segmentSeries, timelineDomainMax, timelineTimeTicks, timelineTooltip, timelineValueTicks, visibleTimelineLines } from './nodeMemoryDiagnostics';

describe('node memory diagnostics helpers', () => {
    it('normalizes fractional SVG zoom timestamps to ordered integer milliseconds', () => {
        const range = integerTimeRange(1786984160362.8027, 1786984160123.1233);

        expect(range).toEqual({ startMs: 1786984160123, endMs: 1786984160363 });
        expect(Number.isInteger(range.startMs)).toBe(true);
        expect(Number.isInteger(range.endMs)).toBe(true);
    });
    ...
    it('excludes hidden diagnostic series from the chart domain and hover inputs', () => {
        const lines = [
            { label: 'Visible', data: [{ timestamp: 2_000, value: 10 }] },
            { label: 'Hidden', data: [{ timestamp: 2_000, value: 100 }] },
        ];
        const visible = visibleTimelineLines(lines, new Set(['Hidden']));

        expect(visible.map(line => line.label)).toEqual(['Visible']);
        expect(timelineDomainMax(visible)).toBe(10.5);
        expect(timelineTooltip(visible, 2_000, 500).values).toEqual([{ label: 'Visible', value: 10, condition: undefined }]);
    });
});
