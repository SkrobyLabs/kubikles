...
export default function PVCActionsMenu({ pvc, isOpen, onOpenChange, onEditYaml, onShowDependencies, onDelete }) {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (buttonRef.current && !buttonRef.current.contains(event.target)) {
                const menu = document.getElementById(`pvc-menu-${pvc.metadata.uid}`);
    ...
    }, [isOpen, pvc.metadata.uid, onOpenChange]);
    ...
    const menu = (
        ...
        >
            ...
            <button
                onClick={(e) => { e.stopPropagation(); handleAction(() => onShowDependencies(pvc)); }}
                className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-[#3d3d3d] flex items-center gap-2"
            >
            ...
            <button
                onClick={(e) => { e.stopPropagation(); handleAction(() => onDelete(pvc)); }}
