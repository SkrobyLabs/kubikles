...
export default function ValidatingWebhookList({ isVisible }) {
    ...
    const handleBulkActionConfirm = useCallback(async (items) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                await DeleteValidatingWebhookConfiguration(name);
    ...
    const handleExportYaml = useCallback(async (items) => {
        ...
        for (const item of items) {
            try {
                const yaml = await GetValidatingWebhookConfigurationYaml(item.metadata?.name);
                entries.push({ namespace: '', name: item.metadata?.name, kind: 'ValidatingWebhookConfiguration', yaml });
            } catch (err) {
                entries.push({ namespace: '', name: item.metadata?.name, kind: 'ValidatingWebhookConfiguration', yaml: `# Failed: ${err}` });
            }
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        try { await SaveYamlBackup(entries, `validatingwebhookconfigurations-backup-${timestamp}.zip`); } catch (err) { if (err?.toString()) alert('Failed: ' + err); }
    }, []);
