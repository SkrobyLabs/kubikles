...
import { DeletePriorityClass, GetPriorityClassYaml, SaveYamlBackup } from '../../../../wailsjs/go/main/App';
...
export default function PriorityClassList({ isVisible }) {
    ...
    const handleExportYaml = useCallback(async (items) => {
        ...
        for (const item of items) {
            try {
                const yaml = await GetPriorityClassYaml(item.metadata?.name);
                entries.push({ namespace: '', name: item.metadata?.name, kind: 'PriorityClass', yaml });
            } catch (err) {
                entries.push({ namespace: '', name: item.metadata?.name, kind: 'PriorityClass', yaml: `# Failed: ${err}` });
            }
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        try { await SaveYamlBackup(entries, `priorityclasses-backup-${timestamp}.zip`); } catch (err) { if (err?.toString()) alert('Failed: ' + err); }
    }, []);
