...
export default function NamespaceActionsMenu({ namespace, isOpen, onOpenChange, onEditYaml, onDelete }) {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (buttonRef.current && !buttonRef.current.contains(event.target)) {
                const menu = document.getElementById(`namespace-menu-${namespace.metadata.uid}`);
    ...
    }, [isOpen, namespace.metadata.uid, onOpenChange]);
    ...
    const menu = (
        <div
            id={`namespace-menu-${namespace.metadata.uid}`}
            className="fixed w-48 bg-[#2d2d2d] border border-[#3d3d3d] rounded-md shadow-lg z-50 py-1"
            style={{ top: position.top, left: position.left }}
            onClick={(e) => e.stopPropagation()}
        >
            <button
                onClick={(e) => { e.stopPropagation(); handleAction(() => onEditYaml(namespace)); }}
            ...
            <button
                onClick={(e) => { e.stopPropagation(); handleAction(() => onDelete(namespace)); }}
