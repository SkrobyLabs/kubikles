...
import { DeleteLimitRange, GetLimitRangeYaml, SaveYamlBackup } from '../../../../wailsjs/go/main/App';
...
export default function LimitRangeList({ isVisible }) {
    ...
    const { handleShowDetails, handleEditYaml, handleShowDependencies, handleDelete } = useLimitRangeActions();
    ...
    const handleBulkActionConfirm = useCallback(async (items) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                await DeleteLimitRange(currentContext, namespace, name);
    ...
    const handleExportYaml = useCallback(async (items) => {
        ...
        for (const item of items) {
            try {
                const yaml = await GetLimitRangeYaml(item.metadata?.namespace, item.metadata?.name);
                entries.push({ namespace: item.metadata?.namespace, name: item.metadata?.name, kind: 'LimitRange', yaml });
            } catch (err) {
                entries.push({ namespace: item.metadata?.namespace, name: item.metadata?.name, kind: 'LimitRange', yaml: `# Failed: ${err}` });
            }
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        try { await SaveYamlBackup(entries, `limitranges-backup-${timestamp}.zip`); } catch (err) { if (err?.toString()) alert('Failed: ' + err); }
    }, []);
