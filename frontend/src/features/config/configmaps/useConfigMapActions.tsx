...
export const useConfigMapActions = (): any => {
    ...
    const handleEditYaml = (configMap: K8sConfigMap): void => {
        ...
        const tabId = `configmap-${currentContext}-${configMap.metadata.namespace}/${configMap.metadata.name}`;
        openTab({
            id: tabId,
    ...
    const handleShowDependencies = (configMap: K8sConfigMap): void => {
        ...
        const tabId = `deps-configmap-${currentContext}-${configMap.metadata.namespace}/${configMap.metadata.name}`;
        openTab({
            id: tabId,
            title: `${configMap.metadata.name}`,
            icon: DocumentTextIcon,
