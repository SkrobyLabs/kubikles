...
export const useSecretActions = (): any => {
    ...
    const handleEditYaml = (secret: K8sSecret): void => {
        ...
        const tabId = `secret-${currentContext}-${secret.metadata.namespace}/${secret.metadata.name}`;
        openTab({
            id: tabId,
    ...
    const handleShowDependencies = (secret: K8sSecret): void => {
        ...
        const tabId = `deps-secret-${currentContext}-${secret.metadata.namespace}/${secret.metadata.name}`;
        openTab({
            id: tabId,
            title: `${secret.metadata.name}`,
            icon: LockClosedIcon,
