...
import { DeleteSecret, GetSecretYaml, SaveYamlBackup } from '../../../../wailsjs/go/main/App';
...
export default function SecretList({ isVisible }) {
    ...
    const handleBulkActionConfirm = useCallback(async (items) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                await DeleteSecret(currentContext, namespace, name);
                results.push({ name, namespace, success: true, message: '' });
                Logger.info('Secret deleted', { namespace, name });
            } catch (err) {
                results.push({ name, namespace, success: false, message: err.toString() });
                Logger.error('Failed to delete secret', { namespace, name, error: err });
            }
    ...
    const handleExportYaml = useCallback(async (items) => {
        ...
        for (const item of items) {
            ...
            const name = item.metadata?.name;
            try {
                const yaml = await GetSecretYaml(namespace, name);
                entries.push({ namespace, name, kind: 'Secret', yaml });
            } catch (err) {
                entries.push({ namespace, name, kind: 'Secret', yaml: `# Failed to fetch YAML: ${err}` });
            }
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        try {
            await SaveYamlBackup(entries, `secrets-backup-${timestamp}.zip`);
        } catch (err) {
