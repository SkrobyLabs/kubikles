...
import { DeleteServiceAccount, GetServiceAccountYaml, SaveYamlBackup } from '../../../../wailsjs/go/main/App';
...
export default function ServiceAccountList({ isVisible }) {
    ...
    const handleExportYaml = useCallback(async (items) => {
        ...
        for (const item of items) {
            try {
                const yaml = await GetServiceAccountYaml(item.metadata?.namespace, item.metadata?.name);
                entries.push({ namespace: item.metadata?.namespace, name: item.metadata?.name, kind: 'ServiceAccount', yaml });
            } catch (err) {
                entries.push({ namespace: item.metadata?.namespace, name: item.metadata?.name, kind: 'ServiceAccount', yaml: `# Failed: ${err}` });
            }
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        try { await SaveYamlBackup(entries, `serviceaccounts-backup-${timestamp}.zip`); } catch (err) { if (err?.toString()) alert('Failed: ' + err); }
    }, []);
