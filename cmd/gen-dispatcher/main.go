...
func generateDispatcher(methods []methodInfo) []byte {
	...
	// The Accelerator has a deliberately closed, hand-written six-operation
	// dispatcher. Keep the generated ordinary dispatcher out of that build so
	// regeneration remains byte-for-byte representative of its own output.
	buf.WriteString("//go:build !accelerator\n\n")
